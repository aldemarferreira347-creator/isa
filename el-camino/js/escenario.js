...
const Escenario = (function () {
    ...
    const CAPITULOS = {
        ...
        // 2 · Los domingos — atardecer rosa sobre islas
        tarde: {
    ...
    function capaInfinita(g, sprite, desp, y, W, alpha) {
        ...
        // Math.floor en vez de Math.round: en pantallas de alta densidad
        // (DPR>1 en móvil) un desp sub-píxel hacía que Math.round dejara
        // exactamente 1px de grieta entre tiras consecutivas — la línea
        // vertical que se veía cruzando el escenario. Floor garantiza que
        // las tiras siempre solapan en vez de abrirse.
        const ry = Math.floor(y);
        while (x < W) {
            g.drawImage(sprite, Math.floor(x), ry);
            x += tira;
        }
