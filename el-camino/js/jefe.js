...
const Jefe = (function () {
    ...
    let activo = false;
    // Vidas reales = número de preguntas en el banco (se fija en iniciar());
    // este valor por defecto sólo cubre el instante antes de esa llamada.
    let vida = 3;
    let x = 0, y = 0;
    ...
    // Hay una respuesta ya dada esperando su desenlace: los botones no
    // aceptan otra hasta que se lance la pregunta siguiente.
    ...
    // ── Temporizadores de la batalla ──
    // Todo lo que pasa aquí va en diferido (emerger, celebrar un acierto,
    // repetir tras un fallo), y ninguno de esos temporizadores se
    // cancelaba nunca. Si el jugador se quedaba sin vidas justo antes de
    // que venciera uno, el temporizador seguía vivo y volvía a sacar la
    // pregunta ENCIMA del panel de derrota, con la batalla ya desactivada.
    // Se registran todos aquí para poder apagarlos de golpe.
    const temporizadores = new Set();

    function tras(ms, fn) {
        const id = setTimeout(() => {
            temporizadores.delete(id);
            // Una batalla desactivada no ejecuta nada pendiente.
            if (!activo) return;
            fn();
        }, ms);
    ...
    function lanzarPregunta() {
        if (!activo || !window.BaseDatosPreguntas) return;
        if (preguntaActual >= window.BaseDatosPreguntas.length) return;
        respondiendo = false;
    ...
    function recibirRespuesta(indiceRespuesta) {
        if (!activo || respondiendo || estado !== 'preguntando') return;
        if (!window.BaseDatosPreguntas) return;
        // Contestar cierra la pregunta en el acto. Sin esto, los botones
        // seguían activos durante los 2,5 s de la respuesta: tocar tres
        // veces seguidas una opción equivocada gastaba las tres vidas de
        // golpe y encolaba tres repeticiones de la misma pregunta.
        respondiendo = true;
        const pregunta = window.BaseDatosPreguntas[preguntaActual];
    ...
    function derrotar() {
        estado = 'derrotado';
        respondiendo = true;   // ya no se acepta ninguna respuesta más
    ...
    function actualizar(dt, jugador) {
        ...
        // Trigger battle cuando el jugador se acerca al jefe.
        // La distancia es la mitad del ancho de pantalla, con un margen
        // mínimo y máximo: en móvil (390px) el trigger es ~300px, en
        // escritorio (1440px) ~500px. Así siempre se activa antes de
        // que el jefe quede fuera de vista por el borde derecho.
        if (estado === 'esperando') {
            const distTrigger = Math.min(500, Math.max(250, window.innerWidth * 0.55));
            const dist = (x - jugador.x);
            if (dist > 0 && dist < distTrigger) {
                detonarBatalla();
            }
        }
    }
    ...
    function desactivar() {
        activo = false;
        // Apagar la batalla tiene que apagar también lo que tenía en
        // cola, o la pregunta siguiente reaparece sola sobre el panel de
        // derrota unos segundos después.
        cancelarTemporizadores();
        respondiendo = false;
    }
