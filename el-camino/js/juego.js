...
const Juego = (function () {
    ...
    let quien = 'carlo';
    // Safe-area superior: se lee una sola vez y se actualiza al girar.
    // Permite que los corazones y vidas del HUD no queden bajo el notch.
    let safeTop = 0;
    function actualizarSafeArea() {
        const div = document.createElement('div');
        div.style.cssText = 'position:fixed;top:env(safe-area-inset-top,0px);left:0;width:1px;height:1px;pointer-events:none;';
        document.body.appendChild(div);
        safeTop = parseFloat(getComputedStyle(div).top) || 0;
        document.body.removeChild(div);
    }
    actualizarSafeArea();
    window.addEventListener('resize', actualizarSafeArea);
    window.addEventListener('orientationchange', actualizarSafeArea);
    ...
    function medir() {
        ...
        // Con el ancho intacto, el cambio SIEMPRE es la barra de
        // direcciones (o la barra de gestos) de un navegador móvil
        // apareciendo/ocultándose al hacer scroll — nunca un giro real
        // de pantalla, que cambia el ancho también. En algunos móviles
        // (Chrome/Samsung Internet con barra + gestos) esa barra suma
        // más de 160px, y ese caso seguía disparando la reconstrucción
        // completa del capítulo: eso es lo que hacía "desaparecer" el
        // escenario a media partida. Con el ancho sin cambios se tolera
        // un salto de altura bastante mayor antes de reconstruir.
        const umbralAltura = deltaW < 2 ? 280 : 160;
        if (prevW > 0 && deltaW < 2 && deltaH < umbralAltura) return;
    ...
    function cargarCapitulo(indice, silencioso) {
        // Un índice fuera de rango dejaba nivel = null y el juego reventaba
        // en la línea siguiente, con la pantalla en negro y sin forma de
        // recuperarse. Pasa de verdad: una partida guardada de una versión
        // anterior (o un localStorage manipulado) puede traer un número de
        // capítulo que ya no existe. Se acota en vez de confiar.
        const total = Niveles.total;
        let i = Math.floor(Number(indice));
        if (!isFinite(i) || i < 0) i = 0;
        if (i >= total) i = total - 1;

        capituloIndice = i;
        nivel = Niveles.cargar(i, H);
        ...
        // En una recarga silenciosa (redimensión/rotación) a media batalla
        // contra Distancia, reiniciar el jefe de golpe le borraba el
        // progreso — parecía que la pelea entera se reseteaba sola. Los
        // callbacks de Jefe ya leen las variables vivas de este cierre
        // (nivel, companero, etc.), así que no hace falta reconstruirlo:
        // basta con dejarlo tal cual sigue.
        const batallaEnCurso = silencioso && nivel.clave === 'pleno' && window.Jefe && Jefe.activo;
        if (!batallaEnCurso) activarJefeParaCapitulo();
        ...
        // "Recoge los N corazones — el portal..." no tiene sentido en el
        // epílogo ("hogar"): no hay corazones ni portal, sólo la casa.
        // Mostrarlo ahí salía como un objetivo sin relación con lo que se
        // ve en pantalla.
        if (!nivel.casa) Subtitulos.mostrar(objetivoTexto());
        ...
        for (const c of nivel.corazones) {
            ...
            if (!portalAbierto && corazonesTomados.size === nivel.corazones.length) {
                if (window.Jefe && Jefe.activo) {
                    const j = (window.GUION && GUION.jefe) || {};
                    Subtitulos.mostrar(j.corazonesListos, { prioridad: 'ahora' });
                } else {
                    abrirPortal();
                }
            }
    ...
    function mostrarUiBatalla(pregunta) {
        ...
        pregunta.opciones.forEach((opcion, i) => {
            ...
            btn.onclick = () => {
                ...
                // contadas por un solo dedo torpe).
                ocultarUiBatalla();
                Jefe.recibirRespuesta(i);
            };
    ...
    function dibujarHud(cx, t) {
        ...
        if (total) {
            ...
            const R = 11, paso = 30;
            // safeTop: espacio extra para el notch de iPhone / barra de estado
            const x0 = 26, y0 = Math.max(30, safeTop + 16);
            const lat = latido.v;
    ...
    function dibujarVidas() {
        ...
        const R = 8.5, paso = 22;
        // y0 sigue a la cápsula de corazones (o si no hay corazones, desde safe-area)
        const baseY = Math.max(30, safeTop + 16);
        const x0 = 26 + R, y0 = baseY + 34;
        g.save();
    ...
    // 'resize' dispara muchas veces seguidas mientras la barra del
    // navegador móvil termina su animación de aparecer/ocultarse —
    // procesar cada evento intermedio podía leer una altura a medio
    // camino y reconstruir el capítulo con ella. Un pequeño debounce deja
    // que la animación termine y sólo se mide la medida final, ya estable.
    let idMedir = 0;
    function medirDiferido() {
        clearTimeout(idMedir);
        idMedir = setTimeout(medir, 180);
    }
    window.addEventListener('resize', medirDiferido);
    window.addEventListener('orientationchange', medirDiferido);
