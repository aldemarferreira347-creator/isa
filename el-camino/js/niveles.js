...
const Niveles = (function () {
    ...
    function construirPleno(H) {
        const s = Math.round(H * 0.74);
        // En móvil con H≈390px, s≈289 y s-420=-131 (fuera de pantalla).
        // El mirador nunca puede estar más arriba del 22% de la altura ni
        // a menos de 160px del suelo — así el jefe y el portal siempre
        // caben en pantalla en cualquier dispositivo.
        const subida = Math.min(420, Math.max(160, Math.round(H * 0.52)));
        const mirador = Math.max(Math.round(H * 0.08), s - subida);
        ...
        const corrientes = [
            ...
            { x: 2400, ancho: 300, y: Math.max(mirador - 190, Math.round(H * 0.05)), alto: (s + 60) - Math.max(mirador - 190, Math.round(H * 0.05)) }
        ];

        // El corazón dentro de la corriente no puede estar más arriba que
        // la mitad del recorrido de subida — en pantallas pequeñas eso era
        // un valor negativo (fuera de pantalla), ahora se limita.
        const corazonCorrienteY = Math.max(mirador + 30, s - Math.round(subida * 0.78));
        ...
        return {
            ...
            corazones: [
                ...
                { x: 2540, y: corazonCorrienteY, indice: 3 },   // dentro de la corriente
            ...
            acompana: true,
            limiteCaida: s + H + 200,
            limiteArriba: Math.min(-10, mirador - 190 - Math.round(H * 0.3)), limiteAbajo: 80
        };
    }
